...
void send_file(int sockfd, int uid)
{
    ...
    while (num != 0)
    {
        ret = read(fd, str + ret, num);
        num -= ret;
    }
    str[size] = '\0';
    close(fd);
    char *tmp = (char *)malloc(sizeof(char) * size * 2);
    base64_encode((const unsigned char *)str, tmp, size);
    ...
    cJSON_AddNumberToObject(json, "n_length", length);
    cJSON_AddStringToObject(json, "filename", filename);
    cJSON_AddStringToObject(json, "base", tmp);
...
void recv_file(cJSON *node)
{
    ...
    int length = cJSON_GetObjectItem(node, "n_length")->valueint;
    char *str = (char *)malloc(sizeof(char) * length);
    strcpy(str, cJSON_GetObjectItem(node, "base")->valuestring);
    char *buf = (char *)malloc(sizeof(char) * f_size);
    //解码
    base64_decode(str, (unsigned char *)buf);
    int ret = 0;
    int num = f_size;
    while (num != 0)
    {
        ...
        if ((sf = recv(sockfd, temp, number, 0)) > 0)
        {
            ...
            case LOOK_PRIVATE:
               /* printf("\n%d\t%d\t%s\t%s\n",cJSON_GetObjectItem(node,"send_id")->valueint,
                cJSON_GetObjectItem(node,"recv_id")->valueint,
                cJSON_GetObjectItem(node,"content")->valuestring,
